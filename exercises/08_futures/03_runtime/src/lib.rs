...
where
    ...
    //     let handle1 = tokio::spawn(first);
    //     let handle2 = tokio::spawn(second);
    // }
{
    let reply = Arc::new(reply);
    // _fixed_reply is the private function that actually sends the Display
    // representations on each listener
