...
// Here we overide the standart implementation of Display
impl std::fmt::Display for TicketNewError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            //* notice write! needs the formatter, the mask and the error!
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        //* In this ex. 1st we match the error then we branch again...
        Err(err) => match err {
